...
export function StatusBar({ onOpenShortcuts, updateAvailable, updateVersion, updateDownloading, updateProgress, onShowUpdate, onCheckForUpdates }: StatusBarProps) {
  ...
  const { branch: activeBranch } = useSessionGitSummary(active?.id ?? null, !!active);
  const isLinkedWorktree = active?.working_directory?.includes('.hermes/worktrees/') ?? false;
    ...
    <div className="status-bar">
      <div className="status-bar-left">
        ...
        {activeBranch && (
          <>
            <span className="status-bar-divider" />
            <span className="status-bar-branch" title={isLinkedWorktree ? `${activeBranch} (linked worktree)\n${active?.working_directory}` : `${activeBranch} (main checkout)`}>
